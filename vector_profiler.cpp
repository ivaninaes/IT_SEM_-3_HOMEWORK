...
bool push_back(subvector *qv, int d) {
    if (qv->top + 1 >= qv->capacity) { 
        qv->capacity *= 2;
        if (qv->capacity == 0) qv->capacity = 1; 
        qv->mas = (int*)realloc(qv->mas, sizeof(int) * qv->capacity); 
    }
    *(qv->mas + qv->top) = d;
    qv->top++; 
    return 1;
...
bool resize(subvector *qv, unsigned int new_capacity) {
    qv->top = std::min(qv->top, new_capacity); 
    qv->mas = (int*)realloc(qv->mas, new_capacity); 
    qv->capacity = new_capacity;
    return 1; 
} 
void shrink_to_fit(subvector *qv) {
    qv->capacity = qv->top;
    qv->mas = (int*)realloc(qv->mas, qv->capacity);
} 
